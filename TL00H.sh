...
echo "请输入开始查询的版本号（推荐从 `cat $PWD/TL00H/NewVerID` `cat $PWD/TL00H/NewVer` 开始）："
read v
echo "请输入结束查询的版本号:"
read e
...
download_url="http://update.hicloud.com:8180/TDS/data/files/p3/s15/G1022/g223"
...
nver=`grep -n "B" $PWD/TL00H/$NewVer.xml | awk '{print $4}'`
NewVer $nver
echo $version >$PWD/TL00H/NewVer
echo $NewVer >$PWD/TL00H/NewVerID
